...
computePathwayScores <- function(rankedMetaClassGenes, pathwayL, referenceGenes) {
  ...
  pathwayScoresDT <- list()
  for (pathway in pathwayL) {
    pathwayName <- gsub('_.*$', '', pathway$addInfo[1, 1])
     pathwayScores <-
        lapply(splitByClass,
           function(DT, refGns)
               runGSAhyper(genes=DT$gene, gsc=pathway, adjMethod="fdr", universe=refGns),
           refGns=referenceGenes)
    classScores <- lapply(pathwayScores,
                          function(path) qnorm(1 - (path$pvalues/2)))

    pathwayScoresDT[[pathwayName]] <- as.data.table(do.call(cbind, classScores))[, pathway := names(classScores[[1]])]
    }
