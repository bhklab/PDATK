...
#'   then calculates the meta-estimate of C-index and D-index. The forestplot of
...
#'
#' # Validate the meta-estimate calculation
#' validationStats <- calcValidationStats(validationCohorts[1],
#'   selectedModels, seqCohorts=c("PCSI", "TCGA", "Kirby"), nthread=2)
#'
...
#' @import survival
#' @export
calcValidationStats <- function(validationCohorts, selectedModels, seqCohorts,
                                nthread, saveDir) {

    probList <- calcPCOSPscores(validationCohorts, selectedModels, nthread)

    constructMetaEstimatesDF(probList, validationCohorts, seqCohorts)
}
#' @rdname calcValidationStats
#' @export
valStats <- calcValidationStats

#' Calculate Dindex, Cindex and cohort meta-estimates from a list of cohorts
#'     and associated probabilities.
#'
#' @param probList A `list` of probability vectors, one for each cohort
#' @param cohortData A `list` of matrices or data.frames containg per cohort
#'   survival data. Columns OS_Stats and OS are assumed to be present!
#' @param seqCohorts A `character` vector with the names of the cohorts
#'   which contain sequencing data. Must matcht he names attribute of  probList
#'   and cohortData.
#' @param hetero A `logical` vector indicating if the `hetero` parameter
...
#' @return
#' A `list` with items:
#'   * `dIndex`, `cIndex`: a data.frame with statistics for each cohort
#'     and summary
#'   * `probabilites`, `isSequencing`: the probabilities used in the
#'     statistical calcualte and wheter or not a respective cohort contains
#'     sequening data.
#'
#' @importFrom survcomp D.index concordance.index combine.est
#' @importFrom reportROC reportROC
...
#'
#' @param validationCohorts A `list` of validation cohorts
#' @param selectedModels A `list` of selected models, as returned by the
#'   `buildPCOSPmodels` function.
#' @param nthread A `numeric` vector containing the integer number of threads
...
#' @md
#' @export
calcPCOSPscores <- function(validationCohorts, selectedModels, nthread) {
