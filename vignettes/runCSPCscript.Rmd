...
## TODO:: Write utility function for setting grid rows/cols
...
ggarrangePlotL(chrScorePlots, 6)
```
...
```{r load_data}
pathwayL <- loadPathwayGMTs(file.path("..", "data"))
names(pathwayL)
```
...
pathwayStatsDT <- computePathwayScores(rankedMetaClassGenes, pathwayL, referenceGenes)
```
