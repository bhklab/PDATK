...
```{r }
biomarkersOfInterest <- c("EGFR", "SNAI2", "KRT81", "GATA6", "CYP3A5", "HNF1A",
                         "REG1A", "REG1B", "REG3A", "CEL")
names(biomarkersOfInterest) <- c(rep("Basal", 3), rep("Classical", 3), rep("Exocrine", 4))
geneBiomarkerScoreL <- lapply(biomarkersOfInterest, computeGeneBiomarkerScores, 
                              cohortsDataL=cohortsDataL,
                              sampleMetaClassDT=sampleMetaClassDT)
names(geneBiomarkerScoreL) <- biomarkersOfInterest
```

```{r }
biomarkerScoreBoxplotL <- boxplotBiomarkerScoresList(geneBiomarkerScoreL, 
                                                     biomarkersOfInterest,
                                                     comparisons)
```
...
```{r }
sampleMetaClassPredDT <- predictSampleMetaClass(compassLogExprMat, binaryRFmodel, topGenesDT)
...
barplotTumorResponse(sampClassPredwSurvivalDT)
```
...
classSurvComparison <- survfit(Surv(OS, OSstatus) ~ predClass, 
                               data=survBasalvClassical)

pValue <- surv_pvalue(classSurvComparison)$pval.txt

barplotTumorResponse(survBasalvClassicalDT, noXaxis=TRUE, pVal=pValue)
```
