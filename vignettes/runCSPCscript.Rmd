...
consensusClusters <- lapply(preprocCohorts,
                            consensusClusterCohort,
                            maxK=5, distance="pearson", method="hc")
```
...
conClustNormals <- lapply(preprocNormals,
                          consensusClusterCohort,
                          maxK=5, distance="pearson", method="hc")
```
...
# 
MSMthresholds <- calcAllMSMthresholds(cohortPairs, allConClusters, allProcCohorts, nthread=15)
```


```{r compare_all_clusters}
allClusterRepro <- calcAllClusterRepro(MSMthresholds, allConClusters, 
                                       allProcCohorts, nthread=15, seed=1987, 
                                       sampleKind="Rounding")
```
