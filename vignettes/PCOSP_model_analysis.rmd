...
chenClassPredictions <- predictClasses(PCOSPpredValCohorts,
    model=chenGeneFuModel)
birnClassPredictions <- predictClasses(PCOSPpredValCohorts,
...
for (i in seq_along(haiderSigScores)) {
  colMData <- colData(haiderClassPredictions[[i]])
  colMData$genefu_score <- NA_real_
  colMData[names(haiderSigScores[[i]]), ] <- haiderSigScores[[i]]
  colData(hairderClassPredictions[[i]]) <- colMData
}
```
