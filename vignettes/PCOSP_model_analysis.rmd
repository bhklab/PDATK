...
trainedPCOSPmodel <- trainModel(PCOSPmodel, numModels=25, minAccuracy=0.6)
...
```{r PCOSP_predictions, message=FALSE, warnings=FALSE}
...
```{r validate_PCOSP_model, messages=FALSE, warnings=FALSE}
...
```{r RLSModel_training}
trainedRLSmodel <- trainModel(RLSmodel, numModels=25)
```
...
have included the `densitityPlotModelComparion` method, which plots a
...
```{r RGAModel_training}
trainedRGAmodel <- trainModel(RGAmodel, numModels=25)
```
...
```{r plotting Model_Comparisons_dindex, fig.wide=TRUE}
...
```{r plotting Model Comparisons_concindex, fig.wide=TRUE}
...
```
```{r forestPlot_subtyped_PCOSP_model}
forestPlot(subtypeValidatedPCOSPmodel, stat='D_index', groupBy='cohort',
    colourBy='subtype', transform='log2')
```
```{r }
