...
The final step in the standard `SurvivalModel` workflow is to compute performance
staistics for the model on the validation data. This can be accomplished using
...
```
```{r validationStats}
knitr::kable(validationStats(validatedPCOSPmodel))
```
Examing the `data.table` from the `validationStats` slot we can see that three
...
```{r PCOSP_D_index_forestplot}
PCOSPcIndexForestPlot <- forestPlot(validatedPCOSPmodel, stat='concordance_index')
PCOSPcIndexForestPlot
```
```{r PCOSP_ROC_curve}
