...
# NOTE: The train/test splits were extracted from the original PCOSP .rda files
#   and are assumed to be random
# It may be worth checking for correlations between training/testing data to
#   ensure this assumption is valid
...
PCOSPtrainingCohorts <- cohortList[grepl('ICGC', names(cohortList))]
...
allValidationCohorts <-
    validationCohorts[!(names(validationCohorts) %in% 'ICGCSEQ')] # For some reason this cohort gives all NA stats?
...
validationStats <- calcValidationStats(validationCohorts,
                                       selectedModels,
...
modelProbabilities <- calcCohortProbabilties(fitClinicalModels, clinicalFeatures)

clinicalModelStats <- calcModelDandCindex(modelProbabilities,
                                               clinicalFeatures,
                                               seqCohorts=c("PCSI", "TCGA"),
                                               model="ICGCclinical")
...
allCohortProbabilites <- calcPCOSPscores(allValidationCohorts, selectedModels, nthread=15)
...
cohortSubtypeStats <- calcCohortSubtypeStats(allCohortProbabilites, allCohortSurvivalData, cohortClasses, seqCohorts=seqCohorts)
