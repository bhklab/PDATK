...
top20PctFeaturesOverall <- getTopFeatures(CSPCmae, numFeats=floor(0.2*length(rownames(CSPCmae)[[1]])), na.rm=TRUE)

featuresOfInterest <- unique(c(top20PctFeaturesOverall, unlist(top20FeaturesPerCohort)))
```

```{r , eval=FALSE}
featuresOfInterestMAE <- CSPCmae[featuresOfInterest, , ]
```
