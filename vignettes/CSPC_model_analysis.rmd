...
sampleSizes <- vapply(experiments(CSPCmae), ncol, numeric(1))
cohortWeights <- sampleSizes / max(sampleSizes)
top20PctFeaturesOverall <- 
    getTopFeatures(CSPCmae, numFeats=floor(0.2*length(rownames(CSPCmae)[[1]])), 
        w=cohortWeights, na.rm=TRUE)
...
normMAE <- normalize(topFeaturesMAE, MARGIN=1, method=c('medianImpute', 'scale'))
```
...
```{r, eval=FALSE}
trainedConClustModel <- trainModel(conClustModel)
```
